...
func (c *client) Create(shadow *models.Shadow) (*models.Shadow, error) {
	...
	if nd, err := c.customClient.CloudV1alpha1().NodeDesires(namespace).Create(desire); err != nil {
		if d, err := c.customClient.CloudV1alpha1().NodeDesires(namespace).Get(name, metav1.GetOptions{}); err == nil && d != nil {
	...
	if nr, err := c.customClient.CloudV1alpha1().NodeReports(namespace).Create(report); err != nil {
		if r, err := c.customClient.CloudV1alpha1().NodeReports(namespace).Get(name, metav1.GetOptions{}); err == nil && r != nil {
