...
func ShadowCreateOrUpdateCacheSet(cache plugin.DataCache, shadow models.Shadow) {
	...
	reportData, err := cache.Get(cachemsg.AllShadowReportCache)
	if err != nil {
		log.L().Error("get shadow cache error", log.Error(err))
		return
	}
	reportMap := map[string]string{}
	err = json.Unmarshal([]byte(reportData), &reportMap)
	if err != nil {
		log.L().Error("unmarshal err", log.Error(err))
		return
	}
	reportMap[shadow.Name] = shadow.ReportStr

	returnData, err := json.Marshal(reportMap)
	if err != nil {
		log.L().Error("Marshal err", log.Error(err))
		return
	}
	err = cache.Set(cachemsg.AllShadowReportCache, string(returnData))
	if err != nil {
		log.L().Error("set report time err", log.Error(err))
		return
	}
