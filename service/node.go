...
func (n *NodeServiceImpl) GetAllShadowReportTime(namespace string, lenNode int) (reportTimeMap, reportMap map[string]string, err error) {
	...
	reportOk, err := n.Cache.Exist(cachemsg.AllShadowReportCache)
	if err != nil {
		return nil, nil, err
	}
	if !reportOk || !reportTimeOk {
		...
		dataReport, err := n.Cache.Get(cachemsg.AllShadowReportCache)
		if err != nil {
			return nil, nil, err
		}
		err = json.Unmarshal([]byte(dataReport), &reportMap)
		if err != nil {
			return nil, nil, err
		}
...
func (n *NodeServiceImpl) SetNodeShadowCache(namespace string) (reportTimeMap, reportMap map[string]string, err error) {
	...
	err = n.Cache.Set(cachemsg.AllShadowReportCache, string(reportData))
	if err != nil {
		return nil, nil, err
	}
	return
