...
func TestDefaultNodeService_List(t *testing.T) {
	...
	mockObject.cache.EXPECT().Get(cachemsg.AllShadowReportCache).Return("{\"node01\":\"{\\\"apps\\\": []}\"}", nil).AnyTimes()
...
func TestFilterNodeService_List(t *testing.T) {
	...
	mockObject.cache.EXPECT().Get(cachemsg.AllShadowReportCache).Return("{\"node01\":\"{\\\"apps\\\": []}\",\"node02\":\"{\\\"apps\\\": []}\"}", nil).AnyTimes()
