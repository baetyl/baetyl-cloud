...
func initActiveAPI(t *testing.T) (*ActiveAPIImpl, *gin.Engine, *gomock.Controller) {
	api := &ActiveAPIImpl{}
	router := gin.Default()
...
func TestNewActiveAPI(t *testing.T) {
	// bad case
	_, err := NewActiveAPI(&config.CloudConfig{})
	assert.Error(t, err)
}
...
func TestActiveAPIImpl_GetResource(t *testing.T) {
	...
	mActive := ms.NewMockActiveService(mockCtl)
	api.ActiveService = mActive
