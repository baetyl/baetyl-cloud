...
func (s *SyncAPIImpl) Report(msg specV1.Message) (*specV1.Message, error) {
	desire, err := s.SyncService.Report(msg.Metadata["namespace"], msg.Metadata["name"], msg.Content.(specV1.Report))
	if err != nil {
		return nil, err
	}
...
func (s *SyncAPIImpl) Desire(msg specV1.Message) (*specV1.Message, error) {
	res, err := s.SyncService.Desire(msg.Metadata["namespace"], msg.Content.(specV1.DesireRequest).Infos, msg.Metadata)
	if err != nil {
		return nil, err
	}
	return &specV1.Message{
		...
		Metadata: msg.Metadata,
		Content:  specV1.DesireResponse{Values: res},
	}, nil
}
