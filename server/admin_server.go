...
func (s *AdminServer) InitRoute() {
	...
	v1 := s.router.Group("v1")
	{
		...
		configs.PUT("/:name", common.WrapperWithLock(s.api.UpdateConfig, s.api.Locker.Lock, s.api.Locker.Unlock))
	...
	}
	{
		...
		certificate.PUT("/:name", common.WrapperWithLock(s.api.UpdateCertificate, s.api.Locker.Lock, s.api.Locker.Unlock))
	...
	}
	{
		...
		nodes.PUT("/:name", common.WrapperWithLock(s.api.UpdateNode, s.api.Locker.Lock, s.api.Locker.Unlock))
	...
	}
	{
		...
		apps.PUT("/:name", common.WrapperWithLock(s.api.UpdateApplication, s.api.Locker.Lock, s.api.Locker.Unlock))
		apps.DELETE("/:name", common.WrapperRaw(s.api.ValidateResourceForDeleting, true), common.Wrapper(s.api.DeleteApplication))
		apps.POST("", common.WrapperRaw(s.api.ValidateResourceForCreating, true), common.WrapperWithLock(s.api.CreateApplication, s.api.Locker.Lock, s.api.Locker.Unlock))
