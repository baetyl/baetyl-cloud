...
func (s *AdminServer) InitRoute() {
	...
	}
	{
		...
		nodes.PUT("/:name/envs", common.Wrapper(s.api.UpdateNodeEnvs))
		nodes.GET("/:name/envs", common.Wrapper(s.api.GetNodeEnvs))
		nodes.GET("/:name/configs/:app", common.Wrapper(s.api.GetNodeSysAppConfigs))
		nodes.GET("/:name/secrets/:app", common.Wrapper(s.api.GetNodeSysAppSecrets))
		nodes.GET("/:name/certificates/:app", common.Wrapper(s.api.GetNodeSysAppCertificates))
		nodes.GET("/:name/registries/:app", common.Wrapper(s.api.GetNodeSysAppRegistries))
	}
	{
		apps := v1.Group("/apps")
